...
public class RESTClient {
	...
	public static ServiceResponse invoke(ServiceContext serviceContext) {
		...
		//add by yuan 2012-08-31
		List<Header> headers = null;//serviceContext.getHeaders();
		if(headers != null){
			for (Header header : headers) {
				String keyName = header.getName();
				if(keyName.contains("timeout"))
					httpClient.getParams().setParameter(header.getName(),Integer.parseInt(header.getValue()));
			}
		}
		try {
